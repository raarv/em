...
acum <- 0
for (i in 1 : length(out1$counts)) {
	acum <- acum + abs((out1$counts[i]/sum(out1$counts)) - (pnorm(out1$breaks[i+1])-pnorm(out1$breaks[i])))
```
\newpage
```{r}
t_muestra <- 100
...
out1 <- hist(z, plot = FALSE)
plot(out1, col = "lightblue", xlab="observaciones", ylab="frecuencia", main="Distribución Poisson")
acum <- 0
for (i in 1 : length(out1$counts)) {
	acum <- acum + abs((out1$counts[i]/sum(out1$counts)) - (pnorm(out1$breaks[i+1])-pnorm(out1$breaks[i])))
```
...
z <- (muestra - media) / sqrt(varianza / t_muestra)
out1 <- hist(z, plot = FALSE)
plot(out1, col = "lightblue", xlab="observaciones", ylab="frecuencia", main="Distribución Gamma")
acum <- 0
for (i in 1 : length(out1$counts)) {
	acum <- acum + abs((out1$counts[i]/sum(out1$counts)) - (pnorm(out1$breaks[i+1])-pnorm(out1$breaks[i])))
```
...
z <- (muestra - media) / sqrt(varianza / t_muestra)
out1 <- hist(z, plot = FALSE)
plot(out1, col = "lightblue", xlab="observaciones", ylab="frecuencia", main="Distribución Beta")
acum <- 0
for (i in 1 : length(out1$counts)) {
	acum <- acum + abs((out1$counts[i]/sum(out1$counts)) - (pnorm(out1$breaks[i+1])-pnorm(out1$breaks[i])))
```
